...
fetch(jsonUrl) // PRODUCCIÓN
  ...
  .then((data) => {
    ...
    const soloValidas = data.doramas.filter(
      (video) =>
    ...
    data.doramas.sort((a, b) => b.fecha - a.fecha); // PRODUCCIÓN
    const fragment = document.createDocumentFragment();
    const generosSet = new Set();
    let peliculasValidas = [];

    data.doramas.forEach((video) => {
...
console.log(document.title);
  ...
  clappr = new Clappr.Player({
    ...
    watermark: BASE_URL + "kdramas.png",
    position: "top-right",
  });
